> **JIW ERP 적용 오버라이드**: 아래 레퍼런스의 `{colors.primary}`(검정 `#000000`)는 본 프로젝트에서 **네이비 `#0a2976`** 로 대체한다 — 1차 *배경*(버튼 배경·사이드바 활성 메뉴 배경·테이블 thead 배경, `bg-primary`/`bg-sidebar-primary`)에만 적용하고 그 위 글자는 흰색. **본문 텍스트는 잉크 블랙(`--foreground` `#000000`)** 을 유지한다. 실제 토큰은 `src/app/globals.css` `:root` 참조.
